...
def ellipse_i(i, hue, rotation):
    center = 500
    rotation = rotation + (i * 2)
    neg_pos = (1 if hue < 180 else -1)
    hue = hue + neg_pos * (i * 3)
    return ellipse(cx=center, cy=center, rx=100 + (i * 3), ry=300 + (i * 2), fill='none',
        stroke=f'hsla({hue}, 80%, 80%, 0.6)', transform=f'rotate({rotation} {center} {center})')
...
def loops():
    ...
    return svg(width=size, height=size, viewBox='0 0 1000 1000', children=[
        ...
        *(ellipse_i(i, hue, rotation) for i in range(iterations))
    ])
